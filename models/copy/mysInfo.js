...
export default class MysInfo {
  static tips = '请使用#扫码登录，以绑定ck后再使用'
  ...
  static async getUid (e, matchMsgUid = true) {
    ...
    if (e.noTips !== true) {
      e.reply([`你还未绑定UID，请先发送：\n原神请使用：【#绑定+你的UID】\n星铁请使用：【*绑定+你的星铁UID】或【#星铁绑定+你的UID】\n以绑定查询目标\n如需查看其它功能请使用：【*扫码登录】注意是用「米游社」扫码注意是用「米游社」扫码，以此绑定ck查询目标\n绑定完成后请使用：\n星铁：*更新面板\n原神：#更新面板\n以此更新你的角色数据\n如需使用其它功能可使用【可可帮助】【#帮助】【*星铁帮助】等查看可用功能`, segment.button([
        { text: '绑定UID', input: '#绑定uid' }
      ])], false, { at: at || true })
    }
  ...
  static async getSelfUid (e) {
    ...
    if (!selfUser.hasCk) {
      if (e.noTips !== true) {
        e.reply('尚未绑定Cookie或Cookie已失效，请使用 #扫码登录 以绑定ck或 #刷新ck 以刷新缓存', segment.button)
      }
      return false
    }
