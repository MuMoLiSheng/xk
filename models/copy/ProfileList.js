...
const ProfileList = {
  ...
  async render (e) {
    ...
    if (!uid) {
      e._replyNeedUid || e.reply([`你还未绑定${player.game === 'sr' ? `“星铁”` :`“原神”` }UID，请先发送：${player.game === 'sr' ? `【*绑定+你的星铁UID】或【#星铁绑定+你的UID】` :`【#绑定+你的UID】` }以此来绑定你的${player.game === 'sr' ? `“星铁”` :`“原神”` }查询目标，绑定完成后 请发送${player.game === 'sr' ? `【*更新面板】` :`【#更新面板】` }以此更新你的${player.game === 'sr' ? `“星铁”` :`“原神”` }角色详情\n如需查看其它功能绑定ck请使用：${player.game === 'sr' ? `【*扫码登录】` :`【#扫码登录】` }注意是用「米游社」扫码\n如需使用其它功能可使用【可可帮助】【#帮助】【*帮助】【#喵喵帮助】等查看可用功能`, new Button(e).bindUid()], false, { at: true })
      return true
    }
