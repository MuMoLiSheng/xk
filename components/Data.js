...
let Data = {
  ...
  * */
  ...
  async importCfg (key) {
    let sysCfg = await Data.importModule(`config/system/${key}_system.js`, 'xk')
    let diyCfg = await Data.importModule(`config/${key}.js`, 'xk')
    if (diyCfg.isSys) {
      console.error(`xk: config/${key}.js无效，已忽略`)
